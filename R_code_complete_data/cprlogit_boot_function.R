...
### COMPARE LOGISTIC VERSUS REGULARISED REGRESSION

logit.penalised.comparison <- function(logit.model,
                                       reg.model,
                                       lambda.min,
                                       data,
                                       outcome,
                                       prob.class=NULL,
                                       round.off.digits=NULL)
{
  ######
  if(is.null(logit.model)|is.null(reg.model))
  {
    stop("The glm(net) objects MUST be specified")
  }
  
  ######
  
  if(is.null(prob.class)) {prob.class <- 0.5}
  
  if(!is.null(prob.class) & ((prob.class < 0 | prob.class >1)))
  {
    stop("The classifying probability MUST lie between 0 and 1")
  }
  
  #######
  if(is.null(round.off.digits)){round.off.digits <- 4}
  ...
  ##### OVERALL MEASURES OF PEFORMANCE
  # COEFFICIENTS
  logit.coefficients <- logit.model$coefficients
  
  # Nagalkerke's R2
  R2.Nagelkerke.stats <- fmsb::NagelkerkeR2(logit.model)
  R2.Nagelkerke.logit <-R2.Nagelkerke.stats$R2
  
  
  # Brier score
  
  brier.score.logit <- brier.score(outcome=as.vector(as.matrix(data%>%dplyr::select(all_of(outcome)))),
                                   pred=logit.model$fitted.values,
                                   threshold=prob.class)
  
  
  ### DISCRIMINATION
  # auc
  auc.logit.info <- suppressMessages(pROC::auc(response=as.vector(as.matrix(data%>%dplyr::select(all_of(outcome)))),
                                               predictor=logit.model$fitted.values))
  
  auc.logit <- as.numeric(gsub("Area under the curve:","",auc.logit.info))
  
  
  # sensitivity
  
  sensitivity.logit <-  InformationValue::sensitivity(as.vector(as.matrix(data%>%dplyr::select(all_of(outcome)))),
  ...
  # specificity
  specificity.logit <-  InformationValue::specificity(as.vector(as.matrix(data%>%dplyr::select(all_of(outcome)))),
                                                      logit.model$fitted.values,
                                                      threshold=prob.class)
  
  
  # misclassfication error
  misclasserror.logit <- InformationValue::misClassError(as.vector(as.matrix(data%>%dplyr::select(all_of(outcome)))),
                                                         logit.model$fitted.values,
                                                         threshold=prob.class)
  ...
  # ppv 
  ppv.logit <- InformationValue::precision(as.vector(as.matrix(data%>%dplyr::select(all_of(outcome)))),
                                           logit.model$fitted.values,
                                           threshold=prob.class)
  ...
  # npv
  npv.logit <-  InformationValue::npv(as.vector(as.matrix(data%>%dplyr::select(all_of(outcome)))),
                                      logit.model$fitted.values,
                                      threshold=prob.class)
  
 
  # concordance  
  concordance.logit <-  InformationValue::Concordance(as.vector(as.matrix(data%>%dplyr::select(all_of(outcome)))),
                                                      logit.model$fitted.values)
  ...
  plr.logit <-  sensitivity.logit/(1-specificity.logit)
                             
                           
  
  # nlr
  
  nlr.logit <- (1-sensitivity.logit)/specificity.logit
  ...
  # CALIBRATION
  # (n.outcome-n.expected)/n.outcome
  
  outcome.vs.expected.ratio.logit <- outcome_expected.scaled.diff(outcome=as.vector(as.matrix(data%>%dplyr::select(all_of(outcome)))),
                                                                  pred=logit.model$fitted.values,
                                                                  threshold.prob=prob.class)
  
  # cox intercept and slope
  cox.int.slope.logit <-  calibration.int.slope(outcome=as.vector(as.matrix(data%>%dplyr::select(all_of(outcome)))),
                                                pred=logit.model$fitted.values)                                  
  ...
  ##### COEFFICIENTS
  reg.coeffiecients <- t(predict(object = reg.model,
                                 s=lambda.min,
                                 type="coefficients"))
  ...
  ##### OVERALL MEASURES OF PEFORMANCE
  # Nagalkerke's R2
  R2.Nagelkerke.reg <- reg.model$dev.ratio[which(reg.model$lambda== min(reg.model$lambda))]
  

  # Brier score
  
  ###
  pred.names <- colnames(t(base::as.matrix(glmnet::coef.glmnet(reg.model))))
  pred.names[-1]
  
  pred.vars <- data.matrix(data %>%
                           dplyr::select(all_of(pred.names[-1])))
  
  outcome.var <-  data.matrix(data %>%
                              dplyr::select(all_of(outcome)))
  
  ###
  preds.reg <-  stats::predict(reg.model, 
                               newx = pred.vars, 
                               type ='response',
                               s=lambda.min)
  
  
  brier.score.reg <-   brier.score(outcome=as.vector(as.matrix(data%>%dplyr::select(all_of(outcome)))),
                                   pred=preds.reg,
                                   threshold=prob.class)
  ...
  ### DISCRIMINATION
  # auc
  gof.reg <- t(glmnet::assess.glmnet(object=reg.model,
                                     newx=pred.vars,
                                     newy=outcome.var,
                                     family="binomial",
                                     s=lambda.min))
  
  auc.reg <- as.numeric(gof.reg[1,"auc"])
  
  
  # sensitivity
  
  sensitivity.reg <-  InformationValue::sensitivity(as.vector(as.matrix(data%>%dplyr::select(all_of(outcome)))),
                                                    preds.reg,
                                                    threshold=prob.class)
  ...
  # specificity
  specificity.reg <-  InformationValue::specificity(as.vector(as.matrix(data%>%dplyr::select(all_of(outcome)))),
                                                    preds.reg,
                                                    threshold=prob.class)
  ...
  # misclassfication error
  misclasserror.reg <- InformationValue::misClassError(as.vector(as.matrix(data%>%dplyr::select(all_of(outcome)))),
                                                       preds.reg,
                                                       threshold=prob.class)
  ...
  # ppv 
  ppv.reg <- InformationValue::precision(as.vector(as.matrix(data%>%dplyr::select(all_of(outcome)))),
                                         preds.reg,
                                         threshold=prob.class)
  ...
  # npv
  npv.reg <-  InformationValue::npv(as.vector(as.matrix(data%>%dplyr::select(all_of(outcome)))),
                                    preds.reg,
                                    threshold=prob.class)
  
  # concordance  
  concordance.reg <-  InformationValue::Concordance(as.vector(as.matrix(data%>%dplyr::select(all_of(outcome)))),
                                                      preds.reg)
  ...
  plr.reg <-  sensitivity.reg/(1-specificity.reg)
  
  
  
  # nlr
  
  nlr.reg <- (1-sensitivity.reg)/specificity.reg
  ...
  outcome.vs.expected.ratio.reg <- outcome_expected.scaled.diff(outcome=as.vector(as.matrix(data%>%dplyr::select(all_of(outcome)))),
                                                                pred=preds.reg,
                                                                threshold.prob=prob.class)
  
  # cox intercept and slope
  cox.int.slope.reg <-  calibration.int.slope(outcome=as.vector(as.matrix(data%>%dplyr::select(all_of(outcome)))),
                                              pred=preds.reg)
  
  
  # Shrinkage / penalisation
  shrinkage <- reg.coeffiecients/logit.coefficients
  shrinkage[1] <- NA_real_
  ...
  R2.Nagelkerke <- as.numeric(data.frame(R2.Nagelkerke.logit,
                                         R2.Nagelkerke.reg))
  
  Brier.score <- as.numeric(data.frame(brier.score.logit,
                                             brier.score.reg))
  
  R2.brier.score.df <- t(data.frame(R2.Nagelkerke,
                                    Brier.score))
  
  colnames(R2.brier.score.df) <- c("logit.model",
                                   "penalised.logit.model")
  
  # PARAMETER VALUES
  coeff.df <- data.matrix(t(rbind(logit.coefficients,
                                    reg.coeffiecients,
                                    shrinkage)))
  
  colnames(coeff.df) <- c("logistic.model",
                          "penalised.logistic.model",
                          "shrinkage")
  
  
  # DISCRIMINATION
  discrimination.logit.values <- as.numeric(data.frame(auc.logit,
                                                       concordance.logit$Concordance,
                                                       sensitivity.logit,
                                                       specificity.logit,
                                                       misclasserror.logit,
                                                       ppv.logit,
  ...
  discrimination.reg.values <- as.numeric(data.frame(auc.reg,
                                                     concordance.reg$Concordance,
                                                     sensitivity.reg,
                                                     specificity.reg,
                                                     misclasserror.reg,
                                                     ppv.reg,
  ...
  statistics.discrim <- c("auc", "concordance", "sensitivity","specificity",
                          "brier.score","ppv","npv","LR+","LR-")
  
  discrimination.df <- data.frame(statistics.discrim,
                                  discrimination.logit.values,
                                  discrimination.reg.values)
  
  colnames(discrimination.df) <- c("Statistic",
                                   "logit.model",
                                   "penalised.logit.model")
  
  # CALIBRATION
  
  outcome.vs.expected.values <- as.numeric(data.frame(outcome.vs.expected.ratio.logit,
                                                      outcome.vs.expected.ratio.reg))
  
  cox.values.logit <- data.frame(cox.int.slope.logit)
  cox.values.reg <- data.frame(cox.int.slope.reg)
  
  statistics.no.vs.ne <- c("n.predicted/n.outcomes",
                           "cox.intercept",
                           "cox.slope")
  
  calibration.df <- cbind(statistics.no.vs.ne,rbind(outcome.vs.expected.values,
                                                    data.frame(cox.values.logit,
                                                               cox.values.reg)))
  
  colnames(calibration.df) <- c("Statistic",
                                "logit.model",
                                "penalised.logit.model")
  ...
  output <- list("parameters"=round(coeff.df,digits=round.off.digits),
                 "overall.measures.of.performance"=round(R2.brier.score.df,round.off.digits),
                 "measures.of.discrimination"=cbind.data.frame("Statistic"=discrimination.df[,1],
                                                               round(discrimination.df[,-1],round.off.digits)),
                 "measures.of.calibration"=cbind.data.frame("Statistic"=calibration.df[,1],
                                                            round(calibration.df[,-1],round.off.digits)))
...
################################################################################
################################################################################
#  FUNCTION FOR   (BLOCKED) BOOTSTRAP
################################################################################
################################################################################
##### First, created blocked bootstrap samples to control for bias/optimism

bootstrap.samples <- function(data,
                              samples,
                              strata=NULL)
{
  message(paste("Conducting bootstrap procedure...."))
  start.time <- Sys.time()
  
  #### Preliminary checks
  if((is.null(data)|is.null(samples))){stop("Please specify dataset and number of
                                        samples to be bootstrapped")}
  #### Further checks
  ##
  if (is.null(strata))
  {
    boot.samples <- rsample::bootstraps(data=data,
                                        times=samples) 
  }
  ##
  if(!is.null(strata))
  ...
  time.taken <- difftime(time1=end.time,
                         time2=start.time,
                         units="sec")
  
  message(paste("Task completed in", round(time.taken,4), "seconds"))
...
################################################################################
################################################################################
# R FUNCTION FOR FITTING LASSO / LOGISTIC MODEL BOOTSRAPPED DATA WITH LOGIT LINK
################################################################################
################################################################################


regularisation.boot.cc.logit <- function(data.boot.complete,
                                         predictors,
                                         outcome,
                                         data.set.no,
                                         ...
                                         seed.input)
{
  ...
  ################################################################################
  dfs.boot.na <- purrr::map(.x=data.set.no, 
                            .f=function(x) as.data.frame(data.boot.complete$splits[[x]]))
  ...
  dfs.boot <- purrr::map(seq(length(data.set.no)),
                         function(k)
                         {
                           na.omit(dfs.boot.na[[k]]%>% dplyr::select(all_of(pred.use),
                                                                     all_of(outcome)))
                         })
  ...
  if(model.options==1)
  {
    pred.vars <- purrr::map(seq(length(data.set.no)),
                            function(j)
                            {
                              data.matrix(dfs.boot[[j]] %>%
                                            dplyr::select(all_of(pred.use)))})
    
    outcome.var <- purrr::map(seq(length(data.set.no)),
                              function(j)
                              {
    ...
    cv.lasso.dev <- lapply(seq(length(data.set.no)),
                           function(j)
                           {
    ...
    best.lambda.dev <- lapply(seq(length(data.set.no)),
                              function(k)
                              {
    ...
    lasso.model <- lapply(seq(length(data.set.no)),
                          function(s)
                          {
    ...
    ##### COEFFICIENTS
    lasso.coefs <- lapply(seq(length(data.set.no)),
                          function(s)
                          {
    ...
    lasso.coef.matrix <- lapply(seq(length(data.set.no)),
                                function(s)
                                {
    ...
    for (j in seq(length(data.set.no)))
    {
    ...
    lasso.coefficients$Bootstrap_no <- seq(length(data.set.no))
    ...
    gofstats.lasso <- lapply(seq(length(data.set.no)),
                             function(s)
                             {
    ...
    # Nagalkerke's R2
    R2.Nagelkerke <- lapply(seq(length(data.set.no)),
                            function(k)
                            {
    ...
    df.gof.lasso$Bootstrap_no <- seq(length(data.set.no))
    ...
    # predictions
    preds.lasso <- lapply(seq(length(data.set.no)),
                          function(k)
                          {
    ...
    if(is.null(thres.prob.classifier))
    {
      optimal.lasso.list <- lapply(seq(length(data.set.no)),
                                   function(k)
                                   {
    ...
    # Brier score
    brier.score.lasso.lists <- purrr::map(seq(length(data.set.no)),
                                          function(x)
                                          {
    ...
    # concordance  
    concordance.lasso.list <- lapply(seq(length(data.set.no)),
                                     function(s)
                                     {
    ...
    # sensitivity
    sensitivity.lasso.list <- lapply(seq(length(data.set.no)),
                                     function(s)
                                     {
    ...
    outcome.vs.expected.ratio.lasso <- lapply(seq(length(data.set.no)),
                                              function(x)
                                              {
    ...
    # cox intercept and slope
    cox.int.slope.lasso <- lapply(seq(length(data.set.no)), 
                                  function(x)
                                  {
    ...
    # specificity
    specificity.lasso.list <- lapply(seq(length(data.set.no)),
                                     function(s)
                                     {
    ...
    # ppv 
    ppv.lasso.list <- lapply(seq(length(data.set.no)),
                             function(s)
                             {
    ...
    # npv
    npv.lasso.list <- lapply(seq(length(data.set.no)),
                             function(s)
                             {
    ...
    plr.lasso.list <- lapply(seq(length(data.set.no)),
                             function(s)
                             {
    ...
    nlr.lasso.list <- lapply(seq(length(data.set.no)),
                             function(s)
                             {
    ...
    for (k in seq(length(data.set.no)))
    {
    ...
    perf.measures.lasso.df <- data.frame(seq(length(data.set.no)),
    ...
    colnames(perf.measures.lasso.df) <-c("Bootstrap_no",
                                         "brier.score",
                                         "concordance",
  ...
  if(model.options==2)
  {
    ...
    logit.results.list <- lapply(seq(length(data.set.no)),
                                 function(p)
                                 {
                                   glm(formula.model,
                                       family=binomial(link=logit),
    ...
    for (i in seq(length(data.set.no)))
    {
    ...
    logit.results.df$Bootstrap_no <- seq(length(data.set.no))
    ...
    # auc
    auc.logit <- lapply(seq(length(data.set.no)),
                        function(s)
                        {
    ...
    # mse
    mse.logit <- lapply(seq(length(data.set.no)),
                        function(s)
                        {
    ...
    # R2 Nagelkerke
    R2.Nagelkerke.logit.lists <- lapply(seq(length(data.set.no)),
                                        function(s)
                                        {
    ...
    # concordance  
    concordance.logit.list <- lapply(seq(length(data.set.no)),
                                     function(s)
                                     {
                                       InformationValue::Concordance(dfs.boot[[s]]%>%dplyr::select(all_of(outcome)),
    ...
    # cox intercept and slope
    cox.int.slope.logit <- lapply(seq(length(data.set.no)), 
                                  function(x)
                                  {
    ...
    if(is.null(thres.prob.classifier))
    {
      optimal.logit.list <- lapply(seq(length(data.set.no)),
                                   function(s)
                                   {
    ...
    brier.score.logit.lists <- purrr::map(seq(length(data.set.no)),
                                          function(x)
                                          {
                                            brier.score(outcome=as.vector(as.matrix(dfs.boot[[x]]%>%dplyr::select(all_of(outcome)))),
    ...
    # sensitivity
    sensitivity.logit.list <- lapply(seq(length(data.set.no)),
                                     function(s)
                                     {
    ...
    # specificity
    specificity.logit.list <- lapply(seq(length(data.set.no)),
                                     function(s)
                                     {
                                       InformationValue::specificity(dfs.boot[[s]]%>%dplyr::select(all_of(outcome)),
                                                                     logit.results.list[[s]]$fitted.values,
    ...
    # outcome vs expected scaled difference
    outcome.vs.expected.ratio.logit <- lapply(seq(length(data.set.no)),
                                              function(x)
                                              {
                                                outcome_expected.scaled.diff(outcome=dfs.boot[[x]]%>%dplyr::select(all_of(outcome)),
                                                                             pred =logit.results.list[[x]]$fitted.values,
    ...
    # ppv 
    ppv.logit.list <- lapply(seq(length(data.set.no)),
                             function(s)
                             {
                               InformationValue::precision(dfs.boot[[s]]%>%dplyr::select(all_of(outcome)),
                                                           logit.results.list[[s]]$fitted.values,
    ...
    # npv
    npv.logit.list <- lapply(seq(length(data.set.no)),
                             function(s)
                             {
                               InformationValue::npv(dfs.boot[[s]]%>%dplyr::select(all_of(outcome)),
                                                     logit.results.list[[s]]$fitted.values,
    ...
    plr.logit.list <- lapply(seq(length(data.set.no)),
                             function(s)
                             {
    ...
    nlr.logit.list <- lapply(seq(length(data.set.no)),
                             function(s)
                             {
    ...
    perf.measures.logit.df <- data.frame(seq(length(data.set.no)),
                                         matrix.auc.logit,
                                         matrix.mse.logit,
    ...
    colnames(perf.measures.logit.df) <-c("Bootstrap_no",
                                         "auc",
                                         "mse",
                                         "R2.Nagelkerke",
                                         "brier.score",
...
################################################################################
################################################################################
#  FUNCTION FOR VECTORISING R FUNCTION regularisation.boot.cc.logit()
################################################################################
################################################################################

# Function for fitting the models

regularisation.fit.cc.logit.link <- function(data.boot.complete,
                                             predictors,
                                             outcome,
                                             data.set.no,
                                             thres.prob.classifier=NULL,
                                             ...
                                             seed.input)
{
    ...
    start.time <- Sys.time()
    ####
    output <- regularisation.boot.cc.logit(data.boot.complete=data.boot.complete,
   ...
   message(paste("Task completed in", round(time.taken,4), "minute(s)"))
...
glm.net.cc.fit <- function(data.boot.complete,
                           predictors,
                           outcome,
                           data.set.no,
                           thres.prob.classifier=NULL,
                           ...
                           seed.input)
{
  ...
  if (!is.null(thres.prob.classifier))
  {
    ...
    output <-  purrr::map(thres.prob.classifier, 
                          function(x)
                          {
                            regularisation.fit.cc.logit.link(data.boot.complete=data.boot.complete,
                                                             predictors=predictors,
                                                             outcome=outcome,
                                                             data.set.no=data.set.no,
...
parameter_estimates.cc <-  function(results,
                                    alpha=NULL)
{
  ##
  ...
  if (purrr::vec_depth(results)==4)
  {
    ...
    if(results[[1]]$model.option==1)
    {
      ...
      {
        soln.reg.df[[i]] <- round(data.frame(results.df.regularised[-1,i],
    ...
    if(results[[1]]$model.option==2)
    {
      ...
      for (i in seq(length(results)))
      {
        soln.log.df[[i]] <- round(data.frame(results.df.logistic[-1,i],
...
################################################################################
################################################################################
#  FUNCTION FOR GENERATING INPUT FOR CALIBRATION PLOT
################################################################################
################################################################################


calibration_plot.inputs <- function(data,
                                    predictors,
                                    outcome,
                                    thres.prob.classifier=NULL,
                                    nfolds=NULL,
...
{
  ###
  message(paste("Starting procedure...."))
  start.time <- Sys.time()
  ...
  {
    if(alpha.param < 0| alpha.param > 1){stop("alpha.prama must be [0,1]")} 
    
  }
  
  ###
  if(is.null(data)){"Provide input dataset"}
  
  ###
  if(is.null(data)){"Provide input dataset"}
  
  ###
  if(is.null(predictors)){"Specify the predictors"}
  ##
  if(is.null(outcome)){"Specify the output"}
  ...
  dfs.boot <-    na.omit(dfs.boot.na%>% 
                           dplyr::select(all_of(pred.use),
                                         all_of(outcome)))
                     
  
  pred.vars <-  as.matrix(dfs.boot %>%
                                dplyr::select(all_of(pred.use)))
  
  outcome.var <- as.matrix(dfs.boot %>%
                                dplyr::select(all_of(outcome.use)))
  ...
  best.lambda.dev <-  cv.lasso.dev$lambda.min
  
  
  # Fit the final models
  lasso.model <-  glmnet::glmnet(x=pred.vars,
                                 y=outcome.var, 
                                 alpha = alpha.param,
                                 family = "binomial",
                                 lambda.min.ratio= best.lambda.dev)
  
  # predictions
  pred.prob.reg <- stats::predict(lasso.model, 
                                newx = pred.vars, 
                                type = 'response')
  
  colnames(pred.prob.reg) <- c("pred.prob.reg")
  
  pred.outcome.reg <- ifelse(pred.prob.reg>=thres.prob.classifier,1,0)
  colnames(pred.outcome.reg) <- c("pred.outcome.reg")
  ...
  logit.results.list <-  glm(formula.model,
                             family=binomial(link=logit),
                             data=dfs.boot)
                  
  
  # predictions
  pred.prob.logit <-  matrix(logit.results.list$fitted.values)
  pred.outcome.logit <- ifelse(pred.prob.logit>=thres.prob.classifier,1,0)
